...
// quantiser.  See src/dsp/resonator/rings_lookup_tables.py for the
...
inline std::array<float, 129> make_fm_frequency_quantizer_table() {
    ...
    while (len < target) {
        ...
        for (size_t i = len; i > max_index + 1; --i) { scale[i] = scale[i - 1]; }
    ...
    for (size_t i = 0; i < result.size(); ++i) { result[i] = static_cast<float>(scale[i]); }
    return result;
}
