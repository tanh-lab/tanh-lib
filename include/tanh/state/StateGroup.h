 ...
 * allow_blocking flag.
 *
 * Certain operations (like string access) may allocate memory, which is not
...
public:
     ...
     * @throws StateGroupNotFoundException if an intermediate group doesn't
     * exist
     *
     ...
     *
     * Creates intermediate groups and the parameter if they don't exist (when
     * create=true).
     *
     * @tparam T Parameter type (double, float, int, bool, std::string, or
     ...
     *
     * @throws StateKeyNotFoundException if create=false and parameter doesn't
     * exist
     * @throws StateGroupNotFoundException if create=false and intermediate
     * group doesn't exist
     *
     * @warning NOT real-time safe - may allocate memory for new
     * parameters/groups
     */
    template <typename T>
     ...
     *
     * @param listener Pointer to the listener (must remain valid while
     * registered)
     *
