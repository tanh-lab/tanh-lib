...
TANH_API void configure_android_bluetooth_session();
...
TANH_API bool set_android_bluetooth_sco(bool enable);

/// Whether SCO is currently enabled.
TANH_API bool is_android_bluetooth_sco_enabled();

/// Enumerate audio devices via Android AudioManager JNI.
/// Falls back to an empty list if the JavaVM has not been set.
TANH_API std::vector<AudioDeviceInfo> enumerate_android_audio_devices(DeviceType type);

/// Returns the Android API level (e.g. 28 for Android 9, 29 for Android 10).
TANH_API int get_android_api_level();
...
TANH_API bool is_android_classic_bluetooth_connected();
...
TANH_API std::string get_android_active_output_device_name(int32_t aaudio_device_id = 0);
...
TANH_API std::string get_android_active_input_device_name(int32_t aaudio_device_id = 0);
