...
just docs                     # Build the docs (Doxygen + Sphinx) into build/docs/docs/sphinx/html
```
...
`docs/` holds the Doxygen → Breathe → Sphinx site (shibuya theme), published to https://tanh-lab.github.io/tanh-lib/ by `.github/workflows/build_docs_and_deploy.yml` on every push to `main`. `TANH_WITH_DOCS=ON` adds the `sphinx-docs` target (never part of ALL). Design notes and guides live under `docs/sphinx/` (reST or MyST Markdown); the README is a short entry point that links there, so long-form material goes into the docs, not the README. `changelog.rst` includes `CHANGELOG.md`, and `getting_started.rst` includes the README's "### CMake options" table (keep that heading and the "### Consume" heading that ends it). `docs/sphinx/api/class`, `api/struct` and the two `*list.rst` files are generated by breathe-apidoc and gitignored. Public API needs Doxygen comments; the reference is generated from `include/tanh/`.
