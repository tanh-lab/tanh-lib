...
void RingBuffer::initialise_with_positions(size_t num_channels,
                                           size_t num_samples) {
...
void RingBuffer::push_sample(size_t channel, float sample) {
    ...
    if (m_is_full[channel]) {
        ...
        if (m_num_valid[channel] < capacity) {
            ++m_num_valid[channel];
        }
    }
    m_is_full[channel] =
        (m_write_pos[channel] == m_read_pos[channel]);
}

float RingBuffer::pop_sample(size_t channel) {
    const size_t capacity = m_buffer.get_num_frames();
    float sample =
        m_buffer.get_read_pointer(channel)[m_read_pos[channel]];
    m_read_pos[channel] = (m_read_pos[channel] + 1) % capacity;
    m_is_full[channel] = false;
    return sample;
}

void RingBuffer::push_block(size_t channel, const float* data,
                            size_t count) {
    const size_t capacity = m_buffer.get_num_frames();
...
float RingBuffer::get_future_sample(size_t channel,
                                    size_t offset) const {
    const size_t capacity = m_buffer.get_num_frames();
...
float RingBuffer::get_past_sample(size_t channel,
                                  size_t offset) const {
    const size_t capacity = m_buffer.get_num_frames();
    size_t pos =
        (m_read_pos[channel] + capacity - offset) % capacity;
    return m_buffer.get_read_pointer(channel)[pos];
}
...
size_t RingBuffer::get_available_samples(size_t channel) const {
    ...
    return (m_write_pos[channel] + capacity - m_read_pos[channel]) %
           capacity;
}
