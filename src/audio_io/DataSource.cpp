...
uint64_t DataSource::read_pcm_frames(float* output, uint64_t frame_count) {
    if (!m_impl || !m_impl->dataSourceInitialised) return 0;

    ma_uint64 frames_read = 0;
    ma_data_source_read_pcm_frames(&m_impl->dataSource,
                                   output,
                                   frame_count,
                                   &frames_read);
    return frames_read;
...
bool DataSource::seek(uint64_t frame_index) {
    if (!m_impl || !m_impl->dataSourceInitialised) return false;

    return ma_data_source_seek_to_pcm_frame(&m_impl->dataSource, frame_index) ==
           MA_SUCCESS;
}
...
uint64_t DataSource::get_total_frames() const {
    ...
    ma_uint64 length = 0;
    ma_data_source_get_length_in_pcm_frames(&m_impl->dataSource, &length);
    return length;
}
...
uint64_t DataSource::get_cursor() const {
    ...
    ma_uint64 cursor = 0;
    ma_data_source_get_cursor_in_pcm_frames(&m_impl->dataSource, &cursor);
    return cursor;
}

bool DataSource::is_valid() const {
    return m_impl && m_impl->dataSourceInitialised;
}
